...
const onlyTitles = recipes.map(function (recipe) {
  return recipe.title;
}); // ['Crepes', ...]

const titlesWithDuration = recipes.map(function (recipe) {
  return `${recipe.title} (${recipe.duration}min)`;
}); // ['Crepes (60min)', ...]

const timePerServing = recipes.map(function (recipe) {
...
const recipesThatOnlyTake60minutesOrLess = recipes.filter(function (recipe) {
  return recipe.duration <= 60;
});

const allRecipesWithMoreThan2Servings = recipes.filter(function (recipe) {
  return recipe.servings > 2;
});

const allRecipesWithTitlesLongerThan12Characters = recipes.filter(function (
  recipe
) {
